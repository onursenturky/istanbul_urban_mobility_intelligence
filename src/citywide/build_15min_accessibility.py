"""Phase 8, step 2: network-based walking accessibility at 5/10/15 minutes.

For every grid cell's walking anchor, ONE cutoff-Dijkstra run (15-minute
cutoff, the validated exact engine from src/network/accessibility.py)
produces the full reachable-node/time set; the 5- and 10-minute results are
DERIVED by sub-filtering that same result (not by re-running Dijkstra),
since a node reachable within 10 minutes is by definition also in the
15-minute reachable set.

Destinations with snap_status == "QUESTIONABLE" (>1000m from the network)
are EXCLUDED from accessibility counts entirely -- reported, not silently
included as if well-anchored.

Unreachable-within-15-minutes is represented as NaN for nearest travel
time, never as a large finite number.
"""
...
def main() -> None:
    print("=" * 72)
    print("Phase 8 step 2: network-based walking accessibility (5/10/15 min)")
    print("=" * 72)
    ...
    dest_ok = dest[dest["snap_status"] == "OK"].copy()
    ...
    for cat in CATEGORIES:
        ...
        dest_nodes_by_cat[cat] = Counter(sub["network_node"].tolist())
        print(f"    {cat}: {len(sub)} usable destinations at {len(dest_nodes_by_cat[cat])} distinct network nodes")

    print("\n[2/4] Running cutoff-Dijkstra for all grid origins (15-min cutoff, single pass)...")
    t0 = time.time()
    ...
    for _, row in anchors.iterrows():
        grid_id, district, origin_node = row["grid_id"], row["district"], row["walking_anchor_node"]
        reachable = accessibility.reachable_from_node(G_walk, origin_node, "travel_time_walk_s", max_cutoff_s)
    ...
    for t_min in THRESHOLDS_MIN:
        ...
        df.to_parquet(APP_DIR / f"grid_accessibility_{t_min}min.parquet")
        print(f"  saved grid_accessibility_{t_min}min.parquet ({len(df)} rows, {len(df.columns)} cols)")

    nearest_df = pd.DataFrame(nearest_time_rows)
    nearest_df.to_parquet(APP_DIR / "grid_nearest_service_times.parquet")
    print(f"  saved grid_nearest_service_times.parquet")
    ...
    (APP_DIR / "accessibility_computation_diagnostics.json").write_text(json.dumps(diag, indent=2, default=str), encoding="utf-8")
